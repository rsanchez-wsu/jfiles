...
public class JFilesServer implements Runnable {
	...
	@SuppressWarnings("resource") //suppress a filewriter being closed elsewhere error
	@Override
	public void run() {
		...
		try (Socket server = serverSocket.accept()) {
			...
			if (history.exists())	{ 
				File temp = new File("tempHistory.txt"); //create a temporary file
				...
				while ((previous = br.readLine()) != null) { //read in data from previous searches
					tempWriter.write(previous); //put data in temp file
				}
				br.close();
				hstWrt = new FileWriter(temp);//suppressed the warning because it's closed elsewhere
				tempWriter.close(); //hstWrt now writes to the temp file for new searches
				history.delete(); //delete the old searchHistory so temp can be renamed
				temp.renameTo(history); //rename the search file to searchHistory 
			}
			String cmd;
